...
def load_model(model_args):
    # 加载预训练的chatglm3-6b的 model config
    ...
    if model_args.ptuning_checkpoint is not None:
        ...
        model = AutoModel.from_pretrained(model_args.model_name_or_path, config=config, trust_remote_code=True)
    # quantization_bit参数指定了量化位数，通常为8位或16位。
    # 量化是将模型中的权重和激活值从浮点数转换为固定精度的数值，以减少内存使用和加速推理过程。
    # 如果有设置quantization则以int数值加载不参与更新的参数，用以节省显存
    if model_args.quantization_bit is not None:
        print(f"Quantized to {model_args.quantization_bit} bit")
        model = model.quantize(model_args.quantization_bit)
    # pt2训练，为要训练的prefix_encoder参数使用更高数值精度的float32
    # 将模型转换为半精度浮点数（半精度浮点数可以提供比全精度浮点数（32位）略低的内存使用和计算速度），
    # 并设置前缀编码器为浮点数
